...
import os
import json
from tools.Integeroverflow.vec2onehot import vec2onehot

dict_AC = {"NULL": 0, "Add": 1, "Sub": 2, "Mul": 3, "SAdd": 4, "SSub": 5, "SMul": 6}

dict_NodeName = {"NULL": 0, "N1": 1, "N2": 2, "N3": 3, "N4": 4, "N5": 5, "N6": 6, "N7": 7, "C1": 8, "C2": 9, "C3": 10,
                 "C4": 11, "C5": 12}

dict_EdgeOpName = {"NULL": 0, "AC": 1, 'AG': 2, "DI": 3, "AD": 4, "SU": 5, "MU": 6, "SAD": 7, "SSU": 8, "SMU": 9,
                   "IF": 10, "GB": 11, "WH": 12, "FR": 13, "RE": 14, "AH": 15, "RG": 16, "IR": 17, "IT": 18}

dict_NodeOpName = {"NULL": 0, "Normal": 1, "Core": 2}
...
def embedding_node(node_attribute_list):
    ...
    node_embedding = []
    node_vec = []
    main_point = ['N1', 'N2', 'N3', 'N4', 'N5', 'N6', 'N7', 'C1', 'C2', 'C3', 'C4', 'C5']
    ...
    for j in range(0, len(node_attribute_list)):
        ...
        if v in main_point:
            ...
            operate = node_attribute_list[j][2].split(",")
            if (len(operate)) > 1:
                for call_vec in range(len(operate)):
                    if call_vec != (len(operate) - 1):
                        if call_vec != 0:
                            tmp_vf0 = str(tmp_vf0) + str(dict_AC[operate[call_vec]]) + ","
                        else:
                            tmp_vf0 = str(dict_AC[operate[call_vec]]) + ","
                    else:
                        tmp_vf0 = str(tmp_vf0) + str(dict_AC[operate[call_vec]])
                    tmp_vfm0 += np.array(list(v2o.nodeAC2vecEmbedding(operate[call_vec])))

            elif len(operate) == 1:
                tmp_vf0 = dict_AC[operate[0]]
                tmp_vfm0 = v2o.nodeAC2vecEmbedding(operate[0])
...
if __name__ == "__main__":
    node = "../../Integeroverflow/graph_data/nodes/12998.sol"
    edge = "../../Integeroverflow/graph_data/edges/12998.sol"
    ...
    v_path = "../../Integeroverflow/graph_data/nodes"
    e_path = "../../Integeroverflow/graph_data/edges"

    fullnodes_ouptput_tmp = open('./results/Integeroverflow_AutoExtract_fullnodes.json', 'w')
    fullnodes_ouptput_gcn = open('./results/Integeroverflow_AutoExtract_fullnodes.txt', 'a')
    contract_name = open("../../Integeroverflow/final_integeroverflow_name.txt")
    contract_label = open("../../Integeroverflow/final_integeroverflow_label.txt")
